/*
 * A3.3 sibling-overflow regression test — the last incomplete audit fix.
 *
 * Setup: build a multi-level tree where the reclaim path triggers
 * sdb_btree_reclaim_internal_up AND the target sibling would overflow after
 * absorbing the survivor. The proper fix must split the sibling and promote
 * a new separator into the grandparent, preserving uniform leaf depth.
 *
 * Failure mode before the fix: reclaim returns SDB_E_BUFFER_TOO_SMALL and the
 * batch aborts. After the fix, reclaim succeeds, verify accepts the tree,
 * every key still resolves.
 *
 * Keys are sized to fill leaves to near-capacity so that a merge triggers
 * overflow rather than fitting comfortably.
 */
...
#define KEY_SIZE ((size_t)320)     /* big keys so leaves fit few entries */
#define VALUE_SIZE ((size_t)64)
#define TOTAL_KEYS ((size_t)512)   /* enough to produce a multi-level tree */
...
int main(void)
{
    ...
    /* Populate with sequential keys; big keys guarantee multi-level tree. */
    ...
    /*
     * Forward-order deletion of the LOWER half of the keyspace: this drains
     * the leftmost subtrees to empty while the upper half stays fully packed.
     * When a left internal collapses (count==0), reclaim must merge its
     * survivor into the RIGHT sibling — which is still full — forcing the
     * split-and-promote path. Before the A3.3 completion this returns
     * SDB_E_BUFFER_TOO_SMALL and the delete aborts.
     */
    ...
    /* Structure must remain valid and hold exactly the upper half. */
    (void)memset(&verify, 0, sizeof(verify));
    assert(sdb_btree_verify(&tree, &verify) == SDB_OK);
    ...
    /* Delete the remainder; tree must end valid and empty. */
    for (index = TOTAL_KEYS / 2U; index < TOTAL_KEYS; ++index) {
        uint8_t key[KEY_SIZE];
        make_key(index, key);
