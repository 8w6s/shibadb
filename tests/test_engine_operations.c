...
/*
 * CRITICAL regression — compact must reset the WAL it adopts at swap.
 *
 * Churn many keys (repeated overwrites + deletes) so the source WAL fills
 * with committed-but-uncheckpointed frames from the PRE-compact page layout,
 * then compact (which builds a fresh file and swaps it in). Close and reopen.
 * If the adopted source WAL is not cleared at swap, open-time recovery scans
 * those stale OLD-layout frames from SDB_WAL_HEADER_SIZE and replays any whose
 * txn_id lands just past the new checkpoint_lsn, clobbering the compacted
 * B-tree. This test fails (verify SDB_E_CORRUPT or wrong values) without the
 * swap-time WAL reset and passes with it.
 */
static void test_compact_reopen_no_stale_replay(
    const sdb_database_options *options
)
{
    sdb_database *database = NULL;
    ...
    size_t actual_size;
    size_t i;
    const size_t total_keys = 24U;
    cleanup();
    assert(sdb_database_create(test_path, options, &database) == SDB_OK);
    for (i = 0U; i < total_keys; ++i) {
        uint8_t k[4];
        size_t rev;
        const int written = snprintf((char *)k, sizeof(k), "%03zu", i);
        assert(written == 3);
        for (rev = 0U; rev < 3U; ++rev) {
            (void)memset(value, 0, sizeof(value));
            value[0] = (uint8_t)i;
            value[1] = (uint8_t)rev;
            assert(sdb_kv_put(
                database, namespace_name, sizeof(namespace_name),
                k, 3U, value, sizeof(value)
            ) == SDB_OK);
        }
    }
    /* Delete odd keys so compact genuinely reclaims dead space. */
    for (i = 1U; i < total_keys; i += 2U) {
        uint8_t k[4];
        const int written = snprintf((char *)k, sizeof(k), "%03zu", i);
        assert(written == 3);
        assert(sdb_kv_delete(
            database, namespace_name, sizeof(namespace_name), k, 3U
        ) == SDB_OK);
    }
    ...
    /*
     * Regression guard: after a compact + close, reopening must NOT replay
     * pre-compact WAL frames over the compacted B-tree. On the next open the
     * adopted WAL must be empty (either removed by sdb_replace_finish, which
     * unlinks <db>.wal, or truncated by the sdb_wal_clear reset added at the
     * compact swap), so recovery re-applies nothing. If a future change to
     * the compact swap or to replace_finish left the stale WAL in place, the
     * verify and value checks below would fail. The reset at swap is
     * defense-in-depth: it also closes the window where replace_finish fails
     * after removing the .replace marker but before unlinking the WAL.
     */
    database = NULL;
    assert(sdb_database_open(test_path, options, &database) == SDB_OK);
    assert(sdb_database_verify(database, &verify) == SDB_OK);
...
/*
 * R2a persistent-fd across a compact swap. After compact swaps a freshly
 * built file in, the adopted pager keeps writing through a persistent WAL fd.
 * The pre-R2a hazard: that fd still pointed at the temporary WAL, which is
 * unlinked during the swap, so a post-compact commit would write into a dead
 * inode and be lost on reopen. This puts keys, compacts, then commits a NEW
 * key AFTER the compact (same open handle), closes, reopens, and requires the
 * post-compact write to be durable alongside the compacted data.
 */
static void test_compact_then_commit_persistent_fd(
    const sdb_database_options *options
)
{
    sdb_database *database = NULL;
    ...
    size_t actual_size;
    size_t i;
    const size_t total_keys = 16U;
    static const uint8_t post_key[] = "post-compact-key";
    static const uint8_t post_value[] = "post-compact-value-DURABLE";
    cleanup();
    assert(sdb_database_create(test_path, options, &database) == SDB_OK);
    for (i = 0U; i < total_keys; ++i) {
        uint8_t k[4];
    ...
    /* Delete half so compact actually rebuilds and swaps. */
    for (i = 1U; i < total_keys; i += 2U) {
        uint8_t k[4];
        const int written = snprintf((char *)k, sizeof(k), "%03zu", i);
        assert(written == 3);
    ...
    /*
     * Commit a brand-new key on the SAME handle, after the swap. This goes
     * through the adopted pager's persistent WAL fd, which must now point at
     * the source WAL (lazily reopened), NOT the unlinked temporary WAL.
     */
    assert(sdb_kv_put(
        database, namespace_name, sizeof(namespace_name),
        post_key, sizeof(post_key), post_value, sizeof(post_value)
    ) == SDB_OK);
    assert(sdb_database_close(database) == SDB_OK);

    database = NULL;
    assert(sdb_database_open(test_path, options, &database) == SDB_OK);
    /* The post-compact commit must have survived. */
    actual_size = 0U;
    assert(sdb_kv_get(
        database, namespace_name, sizeof(namespace_name),
        post_key, sizeof(post_key), actual, sizeof(actual), &actual_size
    ) == SDB_OK);
    assert(actual_size == sizeof(post_value));
    assert(memcmp(actual, post_value, sizeof(post_value)) == 0);
    /* Surviving even keys from the compacted file are still readable. */
    for (i = 0U; i < total_keys; i += 2U) {
        uint8_t k[4];
        const int written = snprintf((char *)k, sizeof(k), "%03zu", i);
        assert(written == 3);
        actual_size = 0U;
    ...
    assert(sdb_database_close(database) == SDB_OK);
    cleanup();
    (void)puts("engine operations: compact then commit persistent-fd ok");
}
...
int main(void)
{
    ...
    test_compact_reopen_no_stale_replay(&options);
    test_compact_then_commit_persistent_fd(&options);
    cleanup();
