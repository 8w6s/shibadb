...
/*
 * These helpers surgically edit a WAL produced by sdb_wal_write_committed,
 * which is the legacy v1/v2 single-record writer: its commit record is 24
 * bytes (SDB_WAL_LEGACY_COMMIT_SIZE), NOT the 44-byte v3 SDB_WAL_COMMIT_SIZE.
 * Using the v3 constant here would locate the commit record 20 bytes past
 * its real start and corrupt the WAL. Kept as a private constant since the
 * legacy size is internal to wal.c.
 */
#define LEGACY_WAL_COMMIT_SIZE ((size_t)24)
...
static void downgrade_wal_to_v1(void)
{
    ...
    assert(size >= SDB_WAL_HEADER_SIZE + LEGACY_WAL_COMMIT_SIZE);
    ...
    commit = bytes + size - LEGACY_WAL_COMMIT_SIZE;
    sdb_write_u16_le(commit + 4U, UINT16_C(1));
    sdb_write_u32_le(
        commit + 16U, sdb_crc32(bytes, size - LEGACY_WAL_COMMIT_SIZE)
    );
    sdb_write_u32_le(commit + 20U, 0U);
    sdb_write_u32_le(
        commit + 20U,
        sdb_crc32_zeroed_range(commit, LEGACY_WAL_COMMIT_SIZE, 20U, 4U)
    );
...
static void duplicate_second_wal_record(void)
{
    ...
    assert(size == SDB_WAL_HEADER_SIZE
        + (2U * record_size) + LEGACY_WAL_COMMIT_SIZE);
    bytes = (uint8_t *)malloc(size);
    assert(bytes != NULL);
    assert(sdb_file_read_full(&file, 0U, bytes, size) == SDB_OK);
    (void)memcpy(
    ...
    sdb_write_u32_le(
        ...
        sdb_crc32_zeroed_range(
            commit, LEGACY_WAL_COMMIT_SIZE, 20U, sizeof(uint32_t)
        )
    );
    assert(sdb_file_write_full(&file, 0U, bytes, size) == SDB_OK);
    assert(sdb_file_sync(&file) == SDB_OK);
