...
**Crash-safety rule (uniform):** No handle in this API — snapshot, cursor, or otherwise — survives a process crash. On restart, every handle is invalid; callers must not persist handles to shared memory across process boundaries. Under Track A snapshots hold no on-disk state, so crash recovery is trivial. Under Track B, dead-owner floor sidecar entries are reclaimed during database open (see the Track B proposal).

---

## 2. Dependency on the concurrency model

**Track A single invariant.** A live `sdb_snapshot` occupies the same *active-session slot* that `sdb_transaction_begin` occupies today (see `docs/PUBLIC_TRANSACTIONS.md`). While the slot is held, every one of the following on the same `sdb_database` handle returns `SDB_E_BUSY`:

- `sdb_transaction_begin`
- `sdb_snapshot_open` (a second snapshot on the same handle)
- every auto-commit mutating call (`sdb_kv_put`, `sdb_kv_delete`, `sdb_blob_*` writes, `sdb_document_*` writes, `sdb_index_*` writes, `sdb_verify`, `sdb_backup`, `sdb_compact`, `sdb_migrate`)
- `sdb_database_close`

This is the *only* invariant Track A relies on. It delivers snapshot isolation by writer exclusion: no committer can advance `checkpoint_lsn` on the handle while the snapshot is live, so every btree page a cursor decodes remains byte-identical to the state at `sdb_snapshot_open` time. Because the process-wide `flock`/`LockFileEx` model already prevents a second handle to the same database file, cross-process interference is not a concern.

**Explicitly not required for Track A.** No reader-floor table, no LSN-tagged freelist, no crash-durable floor sidecar, no read-in-progress refcount for scalar reads. `sdb_pager_free` retains its current immediate-reclaim behaviour. `sdb_kv_get` and other scalar reads continue to run under the existing recursive mutex without a separate refcount.

**Track A trade-off (documented, not hidden).** An auto-commit scan or a long-lived snapshot blocks every writer on the same handle for the duration of the scan. Callers who cannot tolerate that must either (a) page their scan with a bounded `limit`, (b) open a fresh snapshot per page, or (c) wait for the Track B proposal to ship. This proposal makes no attempt to hide the trade-off; §6 auto-commit variants document it as a "writer-blocking operation".

**Track B forward reference.** `PROPOSAL_MULTI_READER_CONCURRENCY.md` will introduce a reader-floor table, LSN-tagged freelist pinning, crash-durable floor sidecar, and a read-in-progress refcount distinct from `callback_depth`. When it ships, the public ABI in this proposal remains stable — `SDB_E_BUSY` on snapshot-vs-writer contention simply becomes a rarer event, and the internal enforcement moves from the active-session flag to per-page LSN checks. Callers who obey the "copy before crossing a state-mutating call" rule (§5.5 Rule 2) are forward-compatible with Track B.
 ...
 *
 * A snapshot holds the database handle's active-session slot (Track A;
 * see §2). While it is held, sdb_transaction_begin, other sdb_snapshot_open
 * calls on the same handle, and every mutating auto-commit call return
 * SDB_E_BUSY. It MUST be closed with sdb_snapshot_close to release the
 * slot; a leaked snapshot leaves the handle wedged against writers.
 * Snapshots hold no on-disk state — a process crash needs no cleanup.
 *
 ...
 * state at the moment of this call and acquires the database handle's
 * active-session slot (Track A; see §2). Under Track B this call will also
 * reserve an LSN floor in the pager's reader-floor table.
 *
 ...
 * @retval SDB_E_BUSY                A transaction or another snapshot is
 *                                   already active on this handle (Track A).
...
/**
 * Release a snapshot, freeing the active-session slot on its parent
 * database handle (Track A). Under Track B this also releases the
 * snapshot's LSN floor.
 *
...
**Snapshot-vs-database lifetime.** Closing the database while any snapshot is live returns `SDB_E_BUSY` from `sdb_database_close`. Under Track A this is enforced via the same active-session flag that gates `transaction_active` in `sdb_database_close` today (see `docs/PUBLIC_TRANSACTIONS.md`). Under Track B it will be enforced via the reader-floor table.
...
**Rule 1 — Snapshot-scoped consistency.**
A cursor observes exactly the committed state at the moment its parent snapshot was opened. Under Track A this is delivered by writer exclusion: while the snapshot is live, no committer can advance `checkpoint_lsn` on the same handle, so every btree page the cursor visits remains byte-identical to its state at `sdb_snapshot_open` time. The process-wide `flock`/`LockFileEx` model already prevents a second handle to the same database file from any other process. Under Track B this same guarantee will be delivered by the reader-floor table plus freelist pinning, permitting concurrent writers on the same handle.
...
Closing the database while any snapshot is live returns `SDB_E_BUSY` from `sdb_database_close`. The invariant is: **database > snapshot ≥ cursor** in reverse lifetime order. Snapshots block database close; cursors do not block snapshot close (they are invalidated instead). Under Track A this is enforced by the same active-session flag that gates `transaction_active` today (`docs/PUBLIC_TRANSACTIONS.md`); under Track B it will be enforced by the reader-floor table.
...
A snapshot and every cursor derived from it must all be touched from the same thread — no exceptions, not even one cursor per thread on a shared snapshot. To scan concurrently, open **one snapshot per thread**. This is a design constraint, not an implementation limitation: the per-cursor decoded-leaf buffers are not synchronized. (Under Track A "one snapshot per thread on a single handle" is moot because the active-session slot admits only one live snapshot per handle at a time. To scan concurrently, open one handle per thread on separate database files, or wait for Track B.)
...
**Rule 8 — Crash safety.**
No cursor or snapshot handle survives a process crash. Placing a handle in shared memory does not extend its lifetime across `exec` or crash-restart. Under Track A snapshots hold no on-disk state, so crash recovery is trivial (the process's `flock` is released by the kernel, next open sees a clean handle). Under Track B, the pager will reclaim reader-floor sidecar entries owned by dead processes during database open.
...
**Auto-commit variants** open an ephemeral snapshot (which takes the handle's active-session slot), walk the range while holding `database->mutex` per navigation step, and close the snapshot at return. Under Track A this is a **writer-blocking operation** for the duration of the walk: any mutating call from another thread on the same handle — or from within the visitor itself — returns `SDB_E_BUSY`. Callers who cannot tolerate a long block must page the scan with a bounded `limit` in `sdb_scan_options` and reissue the call between pages, allowing writers to interleave between scan windows. Under Track B this restriction disappears — the visitor will see a stable snapshot while concurrent writers on the same handle commit freely.
...
| API family | Handle mutex | Active-session slot | Interaction with active transaction |
|---|---|---|---|
| `sdb_snapshot_open` | Briefly | Acquires | Returns `SDB_E_BUSY` if a txn or another snapshot is active. |
| `sdb_transaction_snapshot_open` | Briefly | Inherits txn's slot | Requires active, non-poisoned txn. |
| `sdb_snapshot_close` | Briefly | Releases | Always succeeds. |
| `sdb_cursor_*` navigation | Held per navigation step | Inherited from snapshot | No conflict (snapshot already holds the slot). |
| `sdb_kv_scan_*` (auto-commit) | Held per navigation step | Acquires ephemerally for the whole call | Returns `SDB_E_BUSY` if a txn or snapshot is already active. Writer-blocking under Track A — see §6.3. |
| `sdb_transaction_kv_scan_*` | Via `sdb_transaction_lock` | Inherits txn's slot | Requires active, non-poisoned txn. Merge rules per §6.3. |
| `sdb_index_visit_range` | Via `SDB_ENGINE_LOCK_OR_RETURN` | Acquires ephemerally | Returns `SDB_E_BUSY` if a txn or snapshot is active. Same reentry semantics as `sdb_index_visit`. |
| `sdb_list_*_visit` / `sdb_list_*_into` | Briefly | Not required (catalog reads are metadata, single mutex is enough) | Auto-commit; does not see txn's uncommitted `sdb_transaction_index_create`. |
| `sdb_collection_document_visit` | Held per navigation step | Acquires ephemerally | Same as `sdb_kv_scan_*`. |
...
**Committed-vs-durable under Track A.** Since a snapshot (or ephemeral snapshot inside an auto-commit scan) holds the active-session slot, no writer can advance `checkpoint_lsn` on the same handle during the scan. The "committed state" the scan observes is trivially unambiguous: it is exactly the state as of `sdb_snapshot_open`. The fsync-then-superblock-swap window ambiguity that Track B would need to resolve is moot here — there is no concurrent commit to straddle. Track B will reintroduce this clarification: reads sample `checkpoint_lsn` after WAL fsync, so the *durable* state — any commit `C` for which `wal.fsync(C)` has completed — is what an auto-commit reader observes even during the commit window.
...
- **"Path B implicit snapshot readers can read a page whose plaintext envelope was zeroed by close" (verify #1).** Correct and important under a multi-reader concurrency model. Under Track A the point is moot because scalar reads run under the single recursive mutex, and any pager operation that would zero a plaintext envelope also holds that mutex — no scalar read can straddle it. Track B will resolve this via a read-in-progress refcount distinct from `callback_depth`.
- **"Path B checkpoint/WAL-truncation admits a page_lsn/AAD mismatch under in-place page store" (verify #2).** Correct under a multi-reader concurrency model with in-place writeback. Under Track A this cannot occur because writers are excluded while a snapshot is live. Track B must ship either copy-on-write or WAL pre-image retention; the choice is deferred to `PROPOSAL_MULTI_READER_CONCURRENCY.md`.
...
- **CC-6 "Reverse-iteration reservation is a forward-compat crash risk."** Moot after this revision — reverse iteration is fully shipped in v1 under Track A's active-session-slot exclusion, so there is no future-work window in which a partial reverse implementation could ship. Track B will inherit reverse iteration unchanged.
