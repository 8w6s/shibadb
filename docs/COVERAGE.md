...
Two runs of the `coverage` preset spanning a hardening pass on the
test suite:
1. **Baseline** at commit `aac2445` (41 tests).
2. **Follow-through** at commit `f890a2c` (43 tests, with additional
   coverage-gap fills).
...
- **`abi_symbols`** — `libshibadb.so.0.1.0` under the coverage preset
...
llvm-cov report build-coverage/libshibadb.so.0.1.0 \
