...
An `SDB_E_INVALID_ARGUMENT` or early `SDB_E_BUSY` returned before the database is
acquired does not change the handle's prior state. Once commit acquires the
database, it is terminal even when it returns an error.
In particular, the WAL commit point can already be durable when a later database
sync or checkpoint reports an I/O error. After reopen and recovery the
transaction may therefore be visible. Callers must not interpret such a commit
error as proof of rollback; externally retried operations should carry an
idempotency key. After a terminal commit result, `sdb_transaction_close` releases
the inactive handle.
