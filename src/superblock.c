...
/* Field-level validation failure codes so decode can name the first
 * failing field instead of collapsing every case to SDB_E_CORRUPT.
 * The public API keeps returning SDB_E_CORRUPT; these codes are only
 * used for internal diagnostics and a targeted test surface. */
typedef enum {
    SDB_SB_FIELD_OK = 0,
...
)
{
    ...
    /*
     * Salt is only used to derive the wrap key when encryption is enabled;
     * unencrypted databases are allowed to carry a zero salt per S.7.
     */
    if ((superblock->flags & SDB_FLAG_ENCRYPTED) != 0U
...
#if SDB_TESTING
/* Test surface — gated behind SDB_TESTING so release builds neither
 * export nor carry it. Reachable only by the paired
 * test_superblock_field_diag binary, which is built by presets that
 * set SDB_TESTING=1. Not exposed in any public header. */
...
)
{
    ...
    sdb_write_u16_le(output + 8U, SDB_FORMAT_VERSION_V1);
...
)
{
    ...
    if (sdb_read_u16_le(input + 8U) != SDB_FORMAT_VERSION_V1) {
    ...
    (void)memset(header + SDB_SUPERBLOCK_CHECKSUM_OFFSET, 0, sizeof(uint32_t));
