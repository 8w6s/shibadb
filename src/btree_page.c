...
static int sdb_btree_page_compare_keys(
    const uint8_t *left,
    size_t left_size,
    const uint8_t *right,
    size_t right_size
...
)
{
    ...
    for (index = 1U; index < node.count; ++index) {
        if (sdb_btree_page_compare_keys(
                node.entries[index - 1U].key,
                node.entries[index - 1U].key_size,
...
)
{
    ...
    if (node == NULL || size_out == NULL
        ...
        || (node->count != 0U && node->entries == NULL)
        || (node->kind == SDB_BTREE_LEAF && node->first_child != 0U)
        || (node->kind == SDB_BTREE_INTERNAL
            && (node->first_child == 0U || node->right_sibling != 0U))) {
        return SDB_E_INVALID_ARGUMENT;
    }
    for (index = 0U; index < node->count; ++index) {
        ...
        if (!sdb_checked_add_size(entry_size, entry->key_size, &entry_size)
