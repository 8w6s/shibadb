...
)
{
    ...
    if (handle == INVALID_HANDLE_VALUE) {
        /*
         * Distinguish "the file does not exist yet" from every other
         * open failure so callers that treat the two differently — the
         * WAL recovery path in particular — can propagate real errors
         * instead of masking them as a healthy no-WAL open.
         */
...
{
    /*
     * Parity with POSIX. The prior implementation was a no-op with an
     * inaccurate comment claiming directory handles need backup-semantics
     * privileges — in fact FILE_FLAG_BACKUP_SEMANTICS is just a flag on
     * CreateFileW that any process can pass. Without this call, metadata
     * for a newly-created database, WAL, or replace-marker file could
     * live in the directory-entry cache only; a crash before the
     * filesystem journal flushes that entry loses the file entirely
     * (torn-metadata after FlushFileBuffers on the file body was fine).
     *
     * Steps mirror the POSIX branch below:
     *   1. Extract the parent-directory path via the last path separator.
     *   2. Open the directory with FILE_FLAG_BACKUP_SEMANTICS.
     *   3. FlushFileBuffers on the handle.
     * Failure of any step returns SDB_E_IO. NTFS honours this flush by
     * committing the parent-directory update to the journal.
     */
    ...
    length = strlen(path);
    /* Reject a trailing separator: same rationale as the POSIX branch. */
    if (length > 1U
    ...
    /*
     * Windows accepts both '\\' and '/' as separators. Find the last one
     * of either kind. Fallback: no separator → parent is the current
     * directory.
     */
    {
...
)
{
    ...
    *resolved_path_out = NULL;
    ...
    if (!must_exist) {
        ...
        if (required == 0U
            || (size_t)required > SIZE_MAX / sizeof(*resolved_wide)) {
            free(wide_path);
            return SDB_E_IO;
        }
        resolved_wide = (wchar_t *)malloc(
            (size_t)required * sizeof(*resolved_wide)
        );
        ...
        if (required == 0U
            || (size_t)required + 1U
                > SIZE_MAX / sizeof(*resolved_wide)) {
            (void)CloseHandle(handle);
            return SDB_E_IO;
        }
        resolved_wide = (wchar_t *)malloc(
            ((size_t)required + 1U) * sizeof(*resolved_wide)
        );
        if (resolved_wide == NULL) {
            (void)CloseHandle(handle);
            return SDB_E_INTERNAL;
...
/*
 * Force-select LFS on 32-bit POSIX so uint64_t page offsets do not
 * silently truncate through the (off_t) casts in the pread/pwrite/
 * ftruncate calls below. The CMake target sets _FILE_OFFSET_BITS=64
 * for us; this assert catches a future consumer who drops the flag
 * while linking against the amalgamation.
 */
_Static_assert(
    sizeof(off_t) >= 8,
...
)
{
    ...
    if (descriptor < 0) {
         ...
         * instead of masking them as a healthy no-WAL open.
         */
        return errno == ENOENT ? SDB_E_NOT_FOUND : SDB_E_IO;
    }
...
sdb_status sdb_file_close(sdb_file *file)
{
    ...
    /*
     * Never retry close(2) after EINTR. On several kernels the descriptor
     * has already been released; retrying can close an unrelated descriptor
...
sdb_status sdb_file_sync(sdb_file *file)
{
    ...
    /*
     * fsync(2) on macOS only flushes the kernel buffer to the drive; it does
     * not wait for the drive to flush its on-device cache to media. Use
     * F_FULLFSYNC for a true power-loss barrier (T.7 / S.1). Fall back to
     * fsync on filesystems that reject F_FULLFSYNC (EINVAL / ENOTSUP).
     */
    do {
...
sdb_status sdb_file_sync_parent_directory(const char *path)
{
    ...
    length = strlen(path);
    /*
     * Reject a trailing slash so we cannot fsync the wrong directory when
     * a caller bypasses sdb_file_resolve_database_path.
     */
    ...
    /*
     * As with sdb_file_close, do not retry close after EINTR. The fsync
     * result remains authoritative; a close error is reported only if the
     * durability barrier itself succeeded.
     */
