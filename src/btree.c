...
)
{
    size_t entry_index;
    /*
     * Defense-in-depth: every in-tree caller derives child_index
     * from a lower_bound walk over parent->count, so child_index is
     * always <= node->count by construction. But this is a static
     * helper with `void` return, and a future caller that
     * misinterprets the "child_index == count" boundary (valid: all
     * entries kept, only the rightmost child removed via
     * first_child rewrite path — not this shape) would run past the
     * entries array. Bail out silently on the out-of-range case;
     * callers that need diagnostic status should check bounds
...
/*
 * A non-root internal node `node_page` has collapsed to count==0; its lone
 * surviving child is `survivor_page`. Rebalance by combining the collapsed
 * node with an adjacent sibling under the shared parent, in parent key-order:
 *
 *     combined = [ LEFT subtree ]  <parent separator>  [ RIGHT subtree ]
 *
 * where {LEFT, RIGHT} are node_page and the sibling ordered by their slots
 * under the parent. Then:
 *
 *   - MERGE: if `combined` fits one page, stage it into the sibling page,
 *     drop the collapsed node's slot from the parent, and free node_page.
 *     If the parent itself collapses to count==0, cascade up (or pull the
 *     lone child into the root when the parent is the root).
 *
 *   - SPLIT-AND-PROMOTE: if `combined` overflows, split it back into two
 *     balanced internals reusing both existing pages and promote a fresh
 *     separator into the parent slot in place (parent child count unchanged,
 *     so no cascade). This guarantees reclaim always makes progress instead
 *     of aborting the batch, and uniform leaf depth is preserved on every
 *     path.
 */
...
)
{
    ...
    /* Pick the adjacent sibling and settle parent-order (left,right,sep). */
    if (status == SDB_OK) {
        if (node_slot >= 1U) {
        ...
        /*
         * A corrupt grandparent can produce several aliasing shapes:
         *   sibling_page == node_page      — the two child slots the
         *     reclaimer inspects point at the same page. MERGE would
         *     leave a dangling parent child pointer; SPLIT would collide
         *     on left_page == right_page inside sdb_btree_stage.
         *   sibling_page == 0              — a zero child slot would
         *     read page 0 (the superblock) as if it were a btree node.
         *   sibling_page == parent_page    — immediate parent/child
         *     cycle. Merging or splitting would produce a self-referential
         *     internal node.
         *   survivor_page == parent_page   — the surviving grandchild
         *     is itself the parent, which would embed a cycle into
         *     combined.first_child on the cascade path.
         *   survivor_page == sibling_page  — the surviving grandchild
         *     already belongs to the sibling subtree, so merging would
         *     duplicate its subtree.
         * All are defense-in-depth: reachable only from an already-corrupt
         * on-disk state, and downstream `kind`/`encoded_size` checks would
         * mostly catch the write. Reject up front so the diagnostic path
         * names the fault instead of relying on downstream code to notice.
         */
    ...
    /*
     * Build combined = LEFT ... <sep> ... RIGHT (internal, key-ordered).
     * The collapsed node contributes only survivor_page (as a boundary
     * child); the sibling contributes all its entries. combined then owns
     * the separator key copy plus (shallow) the sibling's entry key/value
     * pointers, so `sibling` is emptied to avoid a double free.
     */
    if (status == SDB_OK) {
    ...
    if (status == SDB_OK) {
        ...
        if (status != SDB_OK) {
            ...
            if (left_page == sibling_page) {
                /* append: [sibling.entries ..., <sep -> survivor>] */
                ...
                /* prepend: [<sep -> sibling.first_child>, sibling.entries ...] */
            ...
            /* Detach sibling's shallow-copied pointers to prevent double free. */
            sibling.count = 0U;
    ...
    if (status == SDB_OK && combined_size <= capacity) {
        /* ---- MERGE: combined fits a single page. ---- */
        ...
        if (status == SDB_OK) {
            if (root_collapse) {
                /* Parent was the root; its sole remaining child is now the
                 * merged node written directly into the root page. Reclaim
                 * the now-unused sibling page. */
                ...
                /* Non-root parent collapsed; its lone child is sibling_page
                 * (holding the merged node). Cascade one level up. */
        ...
        /* ---- SPLIT-AND-PROMOTE: combined overflows one page. ---- */
        ...
        if (status == SDB_OK) {
            /* Views into combined — shallow, no ownership, not destroyed. */
            ...
            if (status == SDB_OK) {
                /* Promote the new separator into the parent slot; the child
                 * pointers on either side of sep_slot already reference
                 * left_page and right_page, so only the key changes. */
...
)
{
    ...
    if (parent_page == batch->tree->root_page) {
        ...
        if (status == SDB_OK && child.kind == SDB_BTREE_LEAF) {
            /* A collapsed root is the only leaf and has no sibling. */
            child.right_sibling = 0U;
        }
        if (status == SDB_OK) {
            /*
             * Reclaim the child page first (it must not appear in the
             * batch anymore before the root re-stage may reuse the
             * slot), then rewrite the root with the child's contents.
             */
        ...
        /*
         * Non-root parent has count==0. Merge parent.first_child (the
         * surviving subtree) into an adjacent sibling under the grandparent
         * so uniform leaf depth is preserved, cascading up as needed.
         */
        status = sdb_btree_reclaim_internal_up(
            batch,
            parent_page,
        ...
        /* Parent is not the root but has no grandparent — corrupt path. */
        status = SDB_E_CORRUPT;
    }
    sdb_btree_node_destroy(&parent);
    return status;
...
)
{
    ...
    for (candidate = 0U; candidate < left->count; ++candidate) {
        ...
        /*
         * 32-bit overflow guard. sdb_btree_batch_put accepts
         * value_size up to UINT32_MAX and key_size up to UINT16_MAX;
         * on a 32-bit host, `6U + key_size + value_size` wraps
         * BEFORE we can check it against SIZE_MAX - total. Route the
         * per-entry math through uint64_t so the wrap is impossible,
         * then narrow only after the range check succeeds.
         */
    ...
    for (candidate = 1U; candidate < left->count; ++candidate) {
        ...
        /*
         * Same 32-bit overflow reasoning. The first loop already
         * validated the wide sum fits in size_t, so we can narrow
         * here without re-checking — but do the addition wide so a
         * future refactor that reorders the loops does not silently
         * regress.
         */
...
)
{
    ...
    /*
     * Both sides must retain at least one entry, else split_internal emits a
     * count==0 half that sdb_btree_verify_node rejects. Restrict the
     * candidate promoted-separator index to [1, count-2] and require count>=3.
     */
    if (left->count < 3U) {
        return SDB_E_BUFFER_TOO_SMALL;
    }
...
)
{
    ...
    sdb_status status;
    /*
     * The caller always owns separator cleanup, including when reading this
     * node fails.  Establish a safe empty result before any fallible work.
     */
    (void)memset(split_out, 0, sizeof(*split_out));
    /*
     * Guard against a corrupt on-disk cycle in the child-pointer graph — a
     * counterpart to the depth cap in sdb_btree_batch_delete.
     */
    if (depth >= 64U) {
        return SDB_E_CORRUPT;
    }
...
sdb_status sdb_btree_create(sdb_pager *pager, sdb_btree *tree_out)
{
    ...
    if (status == SDB_OK) {
        ...
        if (next.generation == UINT64_MAX) {
            ...
            ++next.generation;
            status = sdb_superblock_store_update_file(&pager->file, &next);
            if (status == SDB_OK) {
...
)
{
    ...
    for (;;) {
        ...
        size_t position;
        sdb_status status;
        /* Depth cap parity with write-path (btree.c:1030, 1497). A
         * corrupt on-disk parent chain with a cycle would otherwise
         * spin here reading pages forever. */
        if (depth >= 64U) {
            return SDB_E_CORRUPT;
        }
        ++depth;
    ...
    for (;;) {
        ...
        if (node.kind == SDB_BTREE_LEAF) {
            ...
            if (position == node.count
                ...
                /*
                 * Not-found is a per-op signal, not a batch-poisoning
                 * error. Callers wanting delete-if-exists semantics can
                 * proceed with the batch after receiving SDB_E_NOT_FOUND.
                 */
                sdb_btree_node_destroy(&node);
                return SDB_E_NOT_FOUND;
            }
            ...
            /*
             * Reclaim a leaf that becomes empty only when it is not the
             * root and was not allocated inside this transaction (the
             * pager refuses to free pages it just handed out).
             */
            if (depth > 0U
                && page_id != batch->tree->root_page
...
)
{
    ...
    for (;;) {
        ...
        if (node.kind == SDB_BTREE_LEAF) {
            ...
            if (!cursor->valid && node.right_sibling != 0U) {
...
)
{
    ...
    (void)memset(cursor_out, 0, sizeof(*cursor_out));
    return sdb_btree_cursor_descend(
        tree, key, key_size, true, cursor_out
    );
}
...
sdb_status sdb_btree_cursor_next(sdb_btree_cursor *cursor)
{
    ...
    sibling = cursor->leaf.right_sibling;
    if (sibling == 0U) {
        cursor->valid = false;
        return SDB_OK;
    }
    ...
    cursor->index = 0U;
    cursor->valid = next.count != 0U;
    if (!cursor->valid && next.right_sibling != 0U) {
        return sdb_btree_cursor_next(cursor);
    }
    return SDB_OK;
}
