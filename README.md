...
Embedded ACID key-value database in C11 — encrypted-by-default, single-file,
...
> **Pre-1.0 warning.** shibadb-c is 98% ready per [ROADMAP.md](ROADMAP.md);
> native macOS/Windows release evidence and independent crypto/crash audit
> are pending. **Do not use for production data yet.** See [SECURITY.md](SECURITY.md)
> and [RELEASE.md](RELEASE.md) for the remaining gates.
...
- **Authenticated encryption by default**: XChaCha20-Poly1305 page and WAL
  envelopes, PBKDF2-HMAC-SHA256 key derivation with a 600k-iteration OWASP
  floor, tag verification at recovery time.
...
- **Python binding**: separate repository, wheel-installable, tracks the C
  ABI. See [docs/PYTHON.md](docs/PYTHON.md).

## Platform matrix

| Platform | Compiler           | Status                                        |
|----------|--------------------|-----------------------------------------------|
| Linux    | GCC 12+, Clang 15+ | Verified — full CTest + 10k-op encrypted soak |
| macOS    | Apple Clang        | CI wired, native release evidence pending     |
| Windows  | MSVC 2022          | CI wired, native release evidence pending     |
